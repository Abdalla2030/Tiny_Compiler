...
void Analyze(TreeNode* node, SymbolTable* symbol_table){
    ...
    if(node->node_kind == OPER_NODE){
     ...
     // assign data type ( INTEGER  ) for NUM_NODE and ID_NODE
...
/////////////////////////////////////////////////////////////////////////////
void StartAnalyzing(CompilerInfo* Tiny){
         TreeNode* parse_tree = new TreeNode;
         parse_tree = Parse(Tiny);
         SymbolTable symbol_table;
         Analyze(parse_tree, &symbol_table);
         cout<<("========== Analyzing  ==========")<<endl;
         cout<<endl;
         symbol_table.Print();
         cout<<endl;
...
int calculatePower(int x, int y){ // calculate x power y
    if(x==0){
        return 0;
    }
    if(y==0){
...
/////////////////////////////////////////////////////////////
int checkOper(TreeNode* node, SymbolTable* symbol_table, int* programVariables){
    // base case
    if(node->node_kind == ID_NODE){
            // return the value of variable in RHS
        return programVariables[symbol_table->Find(node->id)->memloc];
    }
    ...
    int x = checkOper(node->child[0], symbol_table, programVariables);
    int y = checkOper(node->child[1], symbol_table, programVariables);

    if(node->oper == EQUAL){ // return true if x and y are equal
            return x == y;
    }
    if(node->oper == LESS_THAN){ // return true if x is less than y
            return x<y;
    }
    if(node->oper == TIMES){ // return x * y
            return x*y;
    }
    if(node->oper == DIVIDE){ // return x / y
            return x/y;
    }
    if(node->oper == POWER){ // return x^y
          return calculatePower(x,y);
    }
    if(node->oper == PLUS){ // return x + y
            return x+y;
    }
    if(node->oper == MINUS){ // return x-y
            return x-y;
    }
   //else
    return 0;
...
void StartCompiling(CompilerInfo* Tiny){
   ...
   cout<<endl;
    ...
    StartingRun(syntax_tree, &symbol_table);
   cout<<"============================="<<endl;
